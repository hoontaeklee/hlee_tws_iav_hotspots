...
fracwet_gdw = xr.open_dataset(path).GDW.values

# wRiver (mm)
path = os.path.join(dir_data, 'SINDBAD_opti_Nov2021/VEG_MSWEP/VEG_3_05_studyArea_wRiver_monthly_masked_simul_2001_2019.nc')  # with mswep
xr_wRiver = xr.open_dataset(path)
wRiver = xr_wRiver.wRiver.values  # in mm
wRiver = np.flip(wRiver, axis=1)
wRiver = wRiver * 1000 * (area * 1000000)  # uncomment to convert to m3
wRiver_max = np.nanmax(wRiver, axis=0)
wRiver_max_log10 = np.log10(wRiver_max)
...
# mask nans
wRiver_max = np.where(common_nonnan, wRiver_max, np.nan)
wRiver_max_log10 = np.where(common_nonnan, wRiver_max_log10, np.nan)
fracwet_gdw = np.where(common_nonnan, fracwet_gdw, np.nan)
...
rescale_x = True
ht_river_sin = get_hist_ht_hot_nhot(data=wRiver_max_log10, hot_pix=hot_pix_sin, region_pix=region_idx, rescale_x=rescale_x)
ht_river_h2m = get_hist_ht_hot_nhot(data=wRiver_max_log10, hot_pix=hot_pix_h2m, region_pix=region_idx, rescale_x=rescale_x)
ht_fracwet_gdw_sin = get_hist_ht_hot_nhot(data=fracwet_gdw, hot_pix=hot_pix_sin, region_pix=region_idx, rescale_x=rescale_x)
ht_fracwet_gdw_h2m = get_hist_ht_hot_nhot(data=fracwet_gdw, hot_pix=hot_pix_h2m, region_pix=region_idx, rescale_x=rescale_x)

# calc. differences
_mb=False  # multiply barwidths or not: rescaled --> False
diff_river_sin = calc_delta_density(dic_ht=ht_river_sin, multiply_barwidth=_mb)
diff_river_h2m = calc_delta_density(dic_ht=ht_river_h2m, multiply_barwidth=_mb)
diff_fracwet_gdw_sin = calc_delta_density(dic_ht=ht_fracwet_gdw_sin, multiply_barwidth=_mb)
diff_fracwet_gdw_h2m = calc_delta_density(dic_ht=ht_fracwet_gdw_h2m, multiply_barwidth=_mb)
...
for i in range(ncol):
    ...
    y_expo = 3 if i==2 else 0  # np.floor(np.log10(y_up)).astype('int')
...
# save
save_name = os.path.join(dir_plot, 'figb22.png')
fig.savefig(
    save_name,
    dpi=600,
    facecolor='w',
