...
# tws iav norm. cov.
path = os.path.join(dir_data, 'cov_TWS_det_scArea_csr06v1.npz')
norm_tws_iav_csr = np.load(path)['norm_tws_iav_csr06v1'].reshape(180, 360)

path = os.path.join(dir_data, 'cov_TWS_det_scArea_det_using_20022017_simul_2001_2019.npz')
cov = np.load(path)
norm_tws_iav_jpl = cov[cov.files[0]].reshape(180, 360)

# apply mask
norm_tws_iav_csr = np.where(common_nonnan, norm_tws_iav_csr, np.nan)
norm_tws_iav_jpl = np.where(common_nonnan, norm_tws_iav_jpl, np.nan)
...
fig = plt.figure(figsize=(11*0.5*1.0, 11*0.5*0.67))
...
# colorbar
cax = plt.axes([0.25, 0.45, 0.6, 0.01])
...
for c in range(nmodels):
    plt.annotate(list_title[c], xy=(1.05, 0.97-c*0.45), xycoords='figure fraction', rotation=90, fontsize=12, va='center', ha='center')

fig.tight_layout()

save_name = os.path.join(dir_plot, 'figb18.png')
fig.savefig(
    save_name,
    dpi=300,
    bbox_inches='tight',
