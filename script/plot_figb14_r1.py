...
# recurrence*(100-occurence): to account for the interaction effect between two variables
occrec = rec * (np.nanmax(occ) - occ) / 100
...
# mask nans
occ = np.where(common_nonnan, occ, np.nan)
occrec = np.where(common_nonnan, occrec, np.nan)
...
rescale_x = True
ht_occ_sin = get_hist_ht_hot_nhot(data=occ, hot_pix=hot_pix_sin, region_pix=region_idx, rescale_x=rescale_x)
ht_occ_h2m = get_hist_ht_hot_nhot(data=occ, hot_pix=hot_pix_h2m, region_pix=region_idx, rescale_x=rescale_x)
ht_occrec_sin = get_hist_ht_hot_nhot(data=occrec, hot_pix=hot_pix_sin, region_pix=region_idx, rescale_x=rescale_x)
ht_occrec_h2m = get_hist_ht_hot_nhot(data=occrec, hot_pix=hot_pix_h2m, region_pix=region_idx, rescale_x=rescale_x)

# calc. differences
_mb=False  # multiply barwidths or not: rescaled --> False
diff_occ_sin = calc_delta_density(dic_ht=ht_occ_sin, multiply_barwidth=_mb)
diff_occ_h2m = calc_delta_density(dic_ht=ht_occ_h2m, multiply_barwidth=_mb)
diff_occrec_sin = calc_delta_density(dic_ht=ht_occrec_sin, multiply_barwidth=_mb)
diff_occrec_h2m = calc_delta_density(dic_ht=ht_occrec_h2m, multiply_barwidth=_mb)
...
# save
save_name = os.path.join(dir_plot, 'figb14.png')
fig.savefig(
    save_name,
    dpi=600,
    facecolor='w',
