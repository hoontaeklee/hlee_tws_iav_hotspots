...
# load T sources by Gonzalo
path = '/Net/Groups/BGI/people/hlee/data/Miguez-Macho_2021_Nature_transpiration_soucres/etsources_global_1x1.nc'
etsrc = xr.open_dataset(path, drop_variables=['latitude', 'longitude'])
idx_month = 12
etsrc_src1 = np.flip(etsrc.SOURCE1.isel(month=idx_month).values, axis=0)
etsrc_src2 = np.flip(etsrc.SOURCE2.isel(month=idx_month).values, axis=0)
etsrc_src3 = np.flip(etsrc.SOURCE3.isel(month=idx_month).values, axis=0)
etsrc_src4 = np.flip(etsrc.SOURCE4.isel(month=idx_month).values, axis=0)
etsrc_nonnan = np.flip(etsrc.AREA.values != 0.0, axis=0)

# get hotspots
norm_cov = np.load(os.path.join(dir_data, 'cov_TWS_det_resid_scArea_det_using_20022017_simul_2001_2019_mswep.npz'))  # forced by mswep
...
# mask nans
etsrc_src1 = np.where(common_nonnan, etsrc_src1, np.nan)
etsrc_src2 = np.where(common_nonnan, etsrc_src2, np.nan)
etsrc_src3 = np.where(common_nonnan, etsrc_src3, np.nan)
etsrc_src4 = np.where(common_nonnan, etsrc_src4, np.nan)
...
rescale_x = True
ht_etsrc_src1_sin = get_hist_ht_hot_nhot(data=etsrc_src1, hot_pix=hot_pix_sin, region_pix=region_idx, rescale_x=rescale_x)
ht_etsrc_src1_h2m = get_hist_ht_hot_nhot(data=etsrc_src1, hot_pix=hot_pix_h2m, region_pix=region_idx, rescale_x=rescale_x)
ht_etsrc_src2_sin = get_hist_ht_hot_nhot(data=etsrc_src2, hot_pix=hot_pix_sin, region_pix=region_idx, rescale_x=rescale_x)
ht_etsrc_src2_h2m = get_hist_ht_hot_nhot(data=etsrc_src2, hot_pix=hot_pix_h2m, region_pix=region_idx, rescale_x=rescale_x)
ht_etsrc_src3_sin = get_hist_ht_hot_nhot(data=etsrc_src3, hot_pix=hot_pix_sin, region_pix=region_idx, rescale_x=rescale_x)
ht_etsrc_src3_h2m = get_hist_ht_hot_nhot(data=etsrc_src3, hot_pix=hot_pix_h2m, region_pix=region_idx, rescale_x=rescale_x)
ht_etsrc_src4_sin = get_hist_ht_hot_nhot(data=etsrc_src4, hot_pix=hot_pix_sin, region_pix=region_idx, rescale_x=rescale_x)
ht_etsrc_src4_h2m = get_hist_ht_hot_nhot(data=etsrc_src4, hot_pix=hot_pix_h2m, region_pix=region_idx, rescale_x=rescale_x)

# calc. differences
_mb=False  # multiply barwidths or not: rescaled --> False
diff_etsrc_src1_sin = calc_delta_density(dic_ht=ht_etsrc_src1_sin, multiply_barwidth=_mb)
diff_etsrc_src1_h2m = calc_delta_density(dic_ht=ht_etsrc_src1_h2m, multiply_barwidth=_mb)
diff_etsrc_src2_sin = calc_delta_density(dic_ht=ht_etsrc_src2_sin, multiply_barwidth=_mb)
diff_etsrc_src2_h2m = calc_delta_density(dic_ht=ht_etsrc_src2_h2m, multiply_barwidth=_mb)
diff_etsrc_src3_sin = calc_delta_density(dic_ht=ht_etsrc_src3_sin, multiply_barwidth=_mb)
diff_etsrc_src3_h2m = calc_delta_density(dic_ht=ht_etsrc_src3_h2m, multiply_barwidth=_mb)
diff_etsrc_src4_sin = calc_delta_density(dic_ht=ht_etsrc_src4_sin, multiply_barwidth=_mb)
diff_etsrc_src4_h2m = calc_delta_density(dic_ht=ht_etsrc_src4_h2m, multiply_barwidth=_mb)
...
# save
save_name = os.path.join(dir_plot, 'figb23.png')
fig.savefig(
    save_name,
    dpi=600,
    facecolor='w',
