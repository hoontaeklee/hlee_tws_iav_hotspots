...
# fluxcom
# mm/day to mm/month: multiply number of days in months for each timestep
date_start = '2002-01-01'
date_end = '2015-12-31'

path = os.path.join(dir_data, 'Fluxcom.ET.RS.EBC-ALL.MLM-ALL.METEO-NONE.360.180.monthly.2001.2015.masked.det.nc')
et_fc = xr.open_dataset(path).sel(time=slice(date_start, date_end))

# convert from mm/d to mm/month
data_years = et_fc.time.dt.year.values
data_months = et_fc.time.dt.month.values
len_of_months = [monthrange(data_years[i], data_months[i])[1] for i in range(len(data_years))]
et_fc['ET_det'] = et_fc['ET_det'] * np.array(len_of_months)[:, None, None]
et_fc = et_fc.ET_det.values

# SINDBAD
path = os.path.join(dir_data, 'sindbad_veg3_det_monthly_180_360_200201_201712.nc')  # forced by gpcp1dd
et_sin = xr.open_dataset(path).evapTotal_det.sel(time=slice(date_start, date_end)).sortby('lat', ascending=False).values

# Hybrid bk
path = os.path.join(dir_data, 'h2m3_det_monthly_180_360_200201_201712.nc')  # forced by gpcp1dd
et_h2m = xr.open_dataset(path).et_det.sel(time=slice(date_start, date_end)).sortby('lat', ascending=False).values
...
# GRUN
path = os.path.join(dir_data, 'Runoff.G-RUN_ENSEMBLE_MMM_v1.360.180.200201.201712.masked.det.nc')
ro_grun = xr.open_dataset(path).sel(time=slice(date_start, date_end))

# convert from mm/d to mm/month
data_years = ro_grun.time.dt.year.values
data_months = ro_grun.time.dt.month.values
len_of_months = [monthrange(data_years[i], data_months[i])[1] for i in range(len(data_years))]
ro_grun['Runoff_det'] = ro_grun['Runoff_det'] * np.array(len_of_months)[:, None, None]
ro_grun = ro_grun.Runoff_det.values

# SINDBAD
path = os.path.join(dir_data, 'sindbad_veg3_det_monthly_180_360_200201_201712.nc')  # forced by gpcp1dd
ro_sin = xr.open_dataset(path).roTotal_det.sel(time=slice(date_start, date_end)).sortby('lat', ascending=False).values

# Hybrid bk
path = os.path.join(dir_data, 'h2m3_det_monthly_180_360_200201_201712.nc')  # forced by gpcp1dd
ro_h2m = xr.open_dataset(path).q_det.sel(time=slice(date_start, date_end)).sortby('lat', ascending=False).values
...
# mask nans
et_fc = np.where(common_nonnan, et_fc, np.nan)
et_sin = np.where(common_nonnan, et_sin, np.nan)
et_h2m = np.where(common_nonnan, et_h2m, np.nan)
ro_grun = np.where(common_nonnan, ro_grun, np.nan)
ro_sin = np.where(common_nonnan, ro_sin, np.nan)
ro_h2m = np.where(common_nonnan, ro_h2m, np.nan)
...
# calc. et bias
et_bias_sin = et_sin - et_fc
et_bias_h2m = et_h2m - et_fc
et_bias_mean_sin = np.nanmean(et_bias_sin, axis=0)
et_bias_mean_h2m = np.nanmean(et_bias_h2m, axis=0)

# calc. runoff bias
ro_bias_sin = ro_sin - ro_grun
ro_bias_h2m = ro_h2m - ro_grun
ro_bias_mean_sin = np.nanmean(ro_bias_sin, axis=0)
ro_bias_mean_h2m = np.nanmean(ro_bias_h2m, axis=0)
# =============================================================================
...
rescale_x = False
ht_et_bias_mean_sin = get_hist_ht_hot_nhot(data=et_bias_mean_sin, hot_pix=hot_pix_sin, region_pix=region_idx, rescale_x=rescale_x)
ht_et_bias_mean_h2m = get_hist_ht_hot_nhot(data=et_bias_mean_h2m, hot_pix=hot_pix_h2m, region_pix=region_idx, rescale_x=rescale_x)
ht_ro_bias_mean_sin = get_hist_ht_hot_nhot(data=ro_bias_mean_sin, hot_pix=hot_pix_sin, region_pix=region_idx, rescale_x=rescale_x)
ht_ro_bias_mean_h2m = get_hist_ht_hot_nhot(data=ro_bias_mean_h2m, hot_pix=hot_pix_h2m, region_pix=region_idx, rescale_x=rescale_x)

# calc. differences
_mb=False  # multiply barwidths or not: rescaled --> False
diff_et_bias_mean_sin = calc_delta_density(dic_ht=ht_et_bias_mean_sin, multiply_barwidth=_mb)
diff_et_bias_mean_h2m = calc_delta_density(dic_ht=ht_et_bias_mean_h2m, multiply_barwidth=_mb)
diff_ro_bias_mean_sin = calc_delta_density(dic_ht=ht_ro_bias_mean_sin, multiply_barwidth=_mb)
diff_ro_bias_mean_h2m = calc_delta_density(dic_ht=ht_ro_bias_mean_h2m, multiply_barwidth=_mb)
# =============================================================================
# plot
# =============================================================================
...
# save
save_name = os.path.join(dir_plot, 'figb17.png')
fig.savefig(
    save_name,
    dpi=600,
    facecolor='w',
