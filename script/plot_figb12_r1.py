    ...
    if draw_bar!=False:
        ...
        if type(bottom) == bool:
            ...
            if bar_ht.shape[0] < 20:
                ...
                bins = np.insert(bins, bins.shape[0], bins[-1])
...
ppt_iav_std_gpcp = np.nanstd(ppt_iav_gpcp, axis=0)
...
ppt_iav_std_mswep = np.nanstd(ppt_iav_mswep, axis=0)
...
tair_iav_std = np.nanstd(tair_iav, axis=0)
...
rn_iav_std = np.nanstd(rn_iav, axis=0)
...
# mask nans
ppt_iav_std_gpcp = np.where(common_nonnan, ppt_iav_std_gpcp, np.nan)
ppt_iav_std_mswep = np.where(common_nonnan, ppt_iav_std_mswep, np.nan)
tair_iav_std = np.where(common_nonnan, tair_iav_std, np.nan)
rn_iav_std = np.where(common_nonnan, rn_iav_std, np.nan)
...
rescale_x = True
ht_ppt_iav_std_gpcp_sin = get_hist_ht_hot_nhot(data=ppt_iav_std_gpcp, hot_pix=hot_pix_sin, region_pix=region_idx, rescale_x=rescale_x)
ht_ppt_iav_std_gpcp_h2m = get_hist_ht_hot_nhot(data=ppt_iav_std_gpcp, hot_pix=hot_pix_h2m, region_pix=region_idx, rescale_x=rescale_x)
ht_ppt_iav_std_mswep_sin = get_hist_ht_hot_nhot(data=ppt_iav_std_mswep, hot_pix=hot_pix_sin, region_pix=region_idx, rescale_x=rescale_x)
ht_ppt_iav_std_mswep_h2m = get_hist_ht_hot_nhot(data=ppt_iav_std_mswep, hot_pix=hot_pix_h2m, region_pix=region_idx, rescale_x=rescale_x)
ht_tair_iav_std_sin = get_hist_ht_hot_nhot(data=tair_iav_std, hot_pix=hot_pix_sin, region_pix=region_idx, rescale_x=rescale_x)
ht_tair_iav_std_h2m = get_hist_ht_hot_nhot(data=tair_iav_std, hot_pix=hot_pix_h2m, region_pix=region_idx, rescale_x=rescale_x)
ht_rn_iav_std_sin = get_hist_ht_hot_nhot(data=rn_iav_std, hot_pix=hot_pix_sin, region_pix=region_idx, rescale_x=rescale_x)
ht_rn_iav_std_h2m = get_hist_ht_hot_nhot(data=rn_iav_std, hot_pix=hot_pix_h2m, region_pix=region_idx, rescale_x=rescale_x)

# calc. differences
_mb=False  # multiply barwidths or not: rescaled --> False
diff_ppt_iav_std_gpcp_sin = calc_delta_density(dic_ht=ht_ppt_iav_std_gpcp_sin, multiply_barwidth=_mb)
diff_ppt_iav_std_gpcp_h2m = calc_delta_density(dic_ht=ht_ppt_iav_std_gpcp_h2m, multiply_barwidth=_mb)
diff_ppt_iav_std_mswep_sin = calc_delta_density(dic_ht=ht_ppt_iav_std_mswep_sin, multiply_barwidth=_mb)
diff_ppt_iav_std_mswep_h2m = calc_delta_density(dic_ht=ht_ppt_iav_std_mswep_h2m, multiply_barwidth=_mb)
diff_tair_iav_std_sin = calc_delta_density(dic_ht=ht_tair_iav_std_sin, multiply_barwidth=_mb)
diff_tair_iav_std_h2m = calc_delta_density(dic_ht=ht_tair_iav_std_h2m, multiply_barwidth=_mb)
diff_rn_iav_std_sin = calc_delta_density(dic_ht=ht_rn_iav_std_sin, multiply_barwidth=_mb)
diff_rn_iav_std_h2m = calc_delta_density(dic_ht=ht_rn_iav_std_h2m, multiply_barwidth=_mb)
...
# set some values
list_dic_ht = [
    'ht_ppt_iav_std_gpcp_sin',
    'ht_ppt_iav_std_gpcp_h2m',
    'ht_ppt_iav_std_mswep_sin',
    'ht_ppt_iav_std_mswep_h2m',
    'ht_tair_iav_std_sin',
    'ht_tair_iav_std_h2m',
    'ht_rn_iav_std_sin',
    'ht_rn_iav_std_h2m'
...
list_lab_data = [
    'PPT IAV Std. GPCP1dd (mm)',
    'PPT IAV Std. MSWEP (mm)',    
    'Tair IAV Std. (degC)',
    'Rn IAV Std. (Wm-2)'
] 
group_name_sel = [
...
# save
save_name = os.path.join(dir_plot, 'figb12.png')
fig.savefig(
    save_name,
    dpi=600,
    facecolor='w',
