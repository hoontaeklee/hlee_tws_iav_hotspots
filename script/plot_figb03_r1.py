...
tws_ano_std_gra = xr.open_dataset(path)['graceTWS_ano_std'].values

path = os.path.join(dir_data, 'sindbad_veg3_det_monthly_180_360_200201_201712.nc')  # forced with GPCP1dd ppt
tws_ano_std_sin = xr.open_dataset(path)['wTotal_ano_std'].sortby('lat', ascending=False).values

path = os.path.join(dir_data, 'h2m3_det_monthly_180_360_200201_201712.nc')  # forced with GPCP1dd ppt
tws_ano_std_h2m = xr.open_dataset(path)['tws_ano_std'].sortby('lat', ascending=False).values

# =============================================================================
# exclude common NaNs
# =============================================================================
tws_ano_std_gra = np.where(common_nonnan, tws_ano_std_gra, np.nan)
tws_ano_std_sin = np.where(common_nonnan, tws_ano_std_sin, np.nan)
tws_ano_std_h2m = np.where(common_nonnan, tws_ano_std_h2m, np.nan)
...
cb.ax.set_xlabel('TWS anomaly std. (mm)', loc='center')
...
save_name = os.path.join(dir_plot, 'figb03.png')
fig.savefig(
    save_name,
    dpi=300,
    bbox_inches='tight',
